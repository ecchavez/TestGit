...
function cargaProcesoInicial()
{
	...
	$('#load_content').load(contexPath+"/UbicacionTS.htm", function(response, status, xhr) {
		  if (status == "success") {
...
{	  
	$("#main_content").empty();
	$("#main_content").html('<div id="pisos-container" style="height: 96%; width: 99%;" align="center"></div>');
...
function cargaProcesoPisoGrid(id_ut_current,id_uts)
{	  
	$("#main_content").empty();
	$("#main_content").html('<div id="imagen-pisos-container" style="height: 96%; width: 99%;"></div>');
	$('#imagen-pisos-container').load(contexPath+'/UbicacionTSGrid.htm?idUbicacion='+id_uts+'&id_ut_current='+id_ut_current+'&utsF='+uts_global, function(response, status, xhr) {
		  if (status == "success") {
	  ...
	  $.ajax({  
	    ...
	    success: function(response){
	      ...
	      if(response.respGetUnidadesTecnicasSuperiores.mensaje == "SUCCESS"){	    	  
	    	  ...
	    	  for(var i=0; i<pisos1; i++)
	    	  {
	    		  ...
	    		  for(var j=0; j<lngDeptos; j++)
		    	  {
	    			  ...
	    			  if(j < deptosxpiso.length)
	    			  {
		    			  ...
		    			  table_detalle+='<td height="26" valign="bottom" align="right">&nbsp;</td>';
		    			  table_detalle+='</tr>';
		    			  table_detalle+='<tr>';
		    			  table_detalle+='<td height="27" colspan="3">&nbsp;</td>';
		    			  table_detalle+='</tr>';
		    			  table_detalle+='<tr>';
		    			  ...
		    			  if(j==0)
		    			  {
		    				  deptos+="<td width=\"35\" align=\"center\" style=\"background: url("+contexPath+"/images/deptos/pestana.png); background-repeat: no-repeat;\">&nbsp;&nbsp;&nbsp;&nbsp;<strong>"+(piso_num)+"</strong>&nbsp;&nbsp;</td><td width=\"0\">"+table_detalle+"</td>";
		    			  }
		    			  else
		    			  {
	    		  ...
	    		  var espacio_tabla="";    		  
	    		  espacio_tabla+='<table width="100%" height="400" border="0" align="center" cellpadding="0" cellspacing="0">';
	    		  espacio_tabla+='<tr>';
	    			  espacio_tabla+='<td>&nbsp;</td>';
	    		  espacio_tabla+='</tr>';
	              espacio_tabla+='</table>';
	    		  ...
	    		  tabla_pisos+=deptos;
	    		  tabla_pisos+="</table><a name=\"bottomlnk\"></a>";  
	    		  
				var tabla_principal='<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" id="grid_principal">';
				tabla_principal+='<td>&nbsp;</td>';
				tabla_principal+='<td height="23">&nbsp;</td>';
				tabla_principal+='</tr>';
				tabla_principal+='<tr>';
				tabla_principal+='<td height="598" valign="bottom"><table border="0" align="center" cellpadding="0" cellspacing="0">';
				tabla_principal+='<tr>';
				tabla_principal+='<td colspan="2" valign="bottom" align="center">'+tabla_pisos+'</td>';
				tabla_principal+='</tr>';
				tabla_principal+='<tr>';
				tabla_principal+='<td height="30" valign="bottom"><table width="100%" height="40" border="0" align="center" cellpadding="0" cellspacing="0">';
				tabla_principal+='<tr>';
				tabla_principal+='<td width="100%" background="'+contexPath+'/images/deptos/cesped_grid.png"><div style="position:relative">';
				tabla_principal+='<div style="position: absolute; width: 100px; left: -19px; top: -103px;">';
				tabla_principal+='<input name="img_arbol2" type="image" src="'+contexPath+'/images/deptos/arbol.png"/>';
				tabla_principal+='</div>';
				tabla_principal+='</div></td>';
				tabla_principal+='</tr>';
				tabla_principal+='</table>';
				tabla_principal+='</td>';
				tabla_principal+='<td>';				  
				tabla_principal+='<table width="100%" height="40" border="0" align="center" cellpadding="0" cellspacing="0">';
				tabla_principal+='<tr>';
				tabla_principal+='<td width="100%" background="'+contexPath+'/images/deptos/cesped_grid.png"><div style="position:relative">';
				tabla_principal+='<div style="position: absolute; left: 452px; top: -100px;">';
				tabla_principal+='<input name="img_arbol2" type="image" src="'+contexPath+'/images/deptos/arbol.png"/>';
				tabla_principal+='</div>';
				tabla_principal+='</div></td>';
				tabla_principal+='</tr>';
		    	...
		    	var cesped_bottom='<div id="cesped_div">';
		    	cesped_bottom+='<div id="arbol_1">';
		    	cesped_bottom+='<input type="image" name="imageField2" id="imageField2" src="'+contexPath+'/images/images/arbolm.png" />';
		    	cesped_bottom+='</div>';
		    	cesped_bottom+='<div id="arbol_2">';
		    	cesped_bottom+='<input type="image" name="imageField" id="imageField" src="'+contexPath+'/images/images/arbol.png" />';
		    	cesped_bottom+='</div>';    
		    	cesped_bottom+='</div>';
		    	
		    	var espacio_tabla1="";    		  
	    		  espacio_tabla1+='<table width="100%" height="20" border="0" align="center" cellpadding="0" cellspacing="0">';
	    		  espacio_tabla1+='<tr>';
	    	  ...
	    	  $("#pisos-container").html('<div id="grid_pisos" align="center">'+espacio_tabla+tabla_pisos+'</div>'+cesped_bottom);
	    	  
	    	  /*var objDiv = document.getElementById("grid_pisos");
...
function getUTSAdmin(accion) {  
	...
	function getDetallesDepto(accion,id_ut_current, id_uts)
	{
		...
		else
		{
			...
			getUbicacionesPisoVenta(id_ut_current,id_uts);
		}
	}
	
	function getUbicacionesPisoVenta(id_ut_current,id_uts)
	{ 
		cargaProcesoPisoGrid(id_ut_current,id_uts);
	...
	function onSelectArbolFiles(e)
	{
		...
		if(nodo_arbol_folder.indexOf(".pdf") != -1)
		{
