...
<title>Consulta de Tickets</title>
    ...
    $(document).ready(function() {
		...
		$("#GridConsultaCatalogoTickets").kendoGrid( {
					 ...
					 reorderable: true,
					 change: function() {
						 var gview = $("#GridConsultaCatalogoTickets").data("kendoGrid");
						 var selectedItem = gview.dataItem(gview.select());
						 if(selectedItem.estatus=='ABIERTO' && validaIngresoModulos("TICKET", "PERMISO_A")){
								window.document.getElementById("darAtencionTicket").style.visibility='';
								window.document.getElementById("darProcesoTicket").style.visibility='';
								$("#darProcesoTicket").prop("disabled", true);
								$("#darProcesoTicket").removeClass("k-button");
								$("#darAtencionTicket").prop("disabled", false);
								$("#darAtencionTicket").addClass("k-button");
								window.document.getElementById("crearImpresionTicket").style.visibility='';
							 }else if((selectedItem.estatus=='EN PROCESO' || selectedItem.estatus=='EJECUTADO') && validaIngresoModulos("TICKET", "PERMISO_M")){
								window.document.getElementById("darAtencionTicket").style.visibility='';
								window.document.getElementById("darProcesoTicket").style.visibility='';
								//window.document.getElementById("crearImpresionTicket").style.visibility='';
								$("#darProcesoTicket").prop("disabled", false);
								$("#darProcesoTicket").addClass("k-button");
								$("#darAtencionTicket").prop("disabled", true);
								$("#darAtencionTicket").removeClass("k-button");
							 }else if(selectedItem.estatus=='CERRADO' || selectedItem.estatus=='RECHAZADO'){
								 $("#darAtencionTicket").prop("disabled", true);
								 $("#darAtencionTicket").removeClass("k-button");
								 $("#darProcesoTicket").prop("disabled", true);
								 $("#darProcesoTicket").removeClass("k-button");
		...
		function crearExcel() {	
		    ...
		    //} else {
		    	setEstatusBar("Generando reporte","mensaje");
		    	//document.getElementById('visorReporte').src= contexPath + "/ticket/GeneraExcel.htm?idTicket=" + model.idTicket;
        ...
        <table width="100%" border="0" cellspacing="0" cellpadding="0">
          ...
          <tr>
            <td height="32"><table width="95%" height="16" border="0" align="center" cellpadding="0" cellspacing="0">
              <tr>
                <td><button class="k-button" style="width:115px" id="seleccionCarCli" onclick="dialogoFiltroConsultaCatalogoTicket()">Seleccionar</button></td>
                <td><a class="k-button" id="verDetalleTicket" style="width:115px; visibility: hidden;" onclick="verDetalleTicket()">Detalle</a></td>
                <td><a class="k-button" id=crearImpresionTicket style="width:115px; visibility: hidden;" onclick="crearImpresionTicket()">Impresi&oacute;n</a></td>
                <td><button class="k-button" id="darAtencionTicket" style="width:115px; visibility: hidden;" onclick="darAtencionTicket()">Atenci&oacute;n</button></td>
                <td><button class="k-button" id="darProcesoTicket" style="width:115px; visibility: hidden;" onclick="darProcesoTicket()">Proceso</button></td>
                <td><a class="k-button" id="crearExcel" style="width:115px; visibility: hidden;" onclick="crearExcel()">Excel</a></td>
                <td>&nbsp;</td>
                <td>&nbsp;</td>
                <td>&nbsp;</td>
                </tr>
...
<div id="windowConsultaTicketDetalle"></div>
<div id="windowAtencionConstruccionTicket"></div>
<div id="windowAtencionTicket"></div>
